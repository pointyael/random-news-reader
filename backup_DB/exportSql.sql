...
CREATE FUNCTION public."getRandomItems"() RETURNS json[]
    LANGUAGE plpgsql
    AS $$DECLARE
...
COPY public.language (lan_id, lan_code, lan_lib) FROM stdin;
1	fr	français
2	en	english
\.
...
COPY public.source (sou_id, sou_name, sou_link) FROM stdin;
1	journal du net	https://www.journaldunet.com
2	random news	https://www.randomnews.com
3	le journal du hard	https://www.lejournalduhard.com
4	lanruoj el	https://www.lanruojel.com
5	gepludide	https://www.gepludide.com
\.
...
COPY public.theme (the_id, the_lib) FROM stdin;
1	sport
2	technologie
\.
...
COPY public.type (typ_id, typ_lib) FROM stdin;
1	image
2	article
\.
